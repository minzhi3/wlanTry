...
public class God implements Callable<GodResult>{
	...
	public GodResult call() throws Exception {
		...
		if (Param.isDebug){
			for (int i=0;i<ThreadNum;i++){
				System.out.println("MT "+i+" "+dm.devices.get(i).x+" "+dm.devices.get(i).y+" "+results.get(i).get().getThroughputRx()+" "+results.get(i).get().getThroughputTx()+" "+results.get(i).get().getDelayString());
			}
		}
		es.shutdown();
